...
class Playlist(asyncio.Queue):
	...
	def embed(self, bot, playing, page):
		"""Creates an embed of the playlist info."""
		embed = discord.Embed(color=0x16dab2)
		embed.set_author(name='Wholesome Music - Now Playing', icon_url=bot.user.avatar_url)

		lines = []

		if playing:
			embed.title = playing.info['title']
			embed.url = playing.info['webpage_url']
			embed.set_thumbnail(url=playing.info['thumbnail'])
			embed.description = f'Requested by {playing.requester.name}.'
		...
		if songs > 15:
			if page > pages:
				are = 'are' if pages > 1 else 'is'
				s = 's' if pages > 1 else ''
				raise commands.CommandError(f'Page number not valid, there {are} {songs} page{s}.')
			start = (page - 1) * 10
			end = min(start + 10, end)
		
		for i in range(start, end):
			lines.append(f'`{start+1}.` **{self._queue[start]}**')
			lines.append(f'Requested by {self._queue[start].requester}.')
		
		if not lines:
			lines.append(f'There are no songs queued! Queue something with {bot.command_prefix}play.')
		
		embed.add_field(name='Queued', value='\n'.join(lines))

		if songs > 15:
			embed.set_footer(text=f'Showing page {page} of {pages} ({songs} total songs).'
				'Use `{bot.command_prefix}queue <page>` to see other pages.')
...
class Music(commands.Cog):
	...
	async def queue(self, ctx, page: int = 1):
		"""Sends the current queue as an embed."""
		await ctx.send(embed=ctx.music_state.playlist.embed(self.bot, ctx.music_state.current_song, page))
